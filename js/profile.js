        ...
        content.innerHTML = `
            ...
            <div class="profile-section">
                ...
                <div class="policies-list" id="policiesList">
                    ${window.renderPoliciesList ? window.renderPoliciesList(policies) : renderPoliciesSimple(policies)}
                </div>
            </div>
...
function renderPoliciesSimple(policies) {
    ...
    return policies.map(policy => {
        ...
        <div class="policy-item" style="margin-bottom: 15px; border-radius: 12px; background: white; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); padding: 15px;">
            ...
            <div><strong>Дата вылета:</strong> ${departureDateStr} ${policy.departure_time || ''}</div>
            ...
            <div class="draft-card" id="draft-${draft.id}" style="margin-bottom: 15px; border-radius: 12px; background: white; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <div class="draft-details" id="draft-details-${draft.id}" style="display: none; padding: 20px; border-top: 1px solid #e0e0e0;">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                        ...
                        <div><strong>Дата вылета:</strong> ${draftData.departureDate || '—'}</div>
                    </div>
                    <div style="display: flex; gap: 10px; justify-content: flex-end;">
                        <button onclick="continueDraft(${draft.id})" style="padding: 8px 20px; background: #667eea; color: white; border: none; border-radius: 8px; cursor: pointer;">
                            <i class="fas fa-edit"></i> Продолжить заполнение
                        </button>
                        <button onclick="deleteDraftById(${draft.id})" style="padding: 8px 20px; background: #f5222d; color: white; border: none; border-radius: 8px; cursor: pointer;">
        ...
        if (data.success && data.notifications.length > 0) {
            notificationsList.innerHTML = data.notifications.map(notif => `
                ...
                " onclick="markNotificationRead(${notif.id})">
                    ...
                    <div style="color: #666; margin-bottom: 8px; word-break: break-word; overflow-wrap: break-word;">${notif.message}</div>
                    ${notif.amount_rub ? `
                    ...
                    ${notif.policy_number ? `
                        <div style="margin-top: 8px; font-size: 12px; color: #999; word-break: break-word;">
                            <i class="fas fa-file-contract"></i> Полис: ${notif.policy_number}
                        </div>
                    ` : ''}
                </div>
            `).join('');
